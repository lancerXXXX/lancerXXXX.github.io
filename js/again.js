...
function switchTreeOrIndex() {
	$("#search-icon").on("click", function (e) {
		$("#tree").animate({ height: 'toggle' }, 100);
...
function showArticleIndex() {
	...
	for (var i = 0; i < labelList.length; i++) {
		if ($(labelList[i]).is("h1")) {
			console.log('添加节点: ' + labelList[i].innerHTML + '  ' + i);
			h2List = new Array();
...
function pjaxLoad() {
	$(document).pjax('#tree a', '#content-outer', { fragment: '#content-outer', timeout: 8000 });
	$(document).pjax('#menu a', '#content-outer', { fragment: '#content-outer', timeout: 8000 });
